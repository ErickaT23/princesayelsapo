(function () {
    ...
    function updateSyncMeta(sourceLabel) {
    ...
    function refreshView() {
        ...
        updateSyncMeta("tiempo real");
    }

    async function forceSyncNow() {
        if (!state.db || !state.eventId) {
            setStatus("No hay conexion para sincronizar.", true);
            return;
        }

        try {
            setStatus("Sincronizando manualmente...", false);
            const [confirmations, invitados] = await Promise.all([
                state.db.getAllConfirmations(state.eventId),
                state.db.getInvitados(state.eventId)
            ]);

            state.confirmations = Array.isArray(confirmations) ? confirmations : [];
            state.invitadosMap = mapInvitados(invitados);
            refreshView();
            setStatus("Sincronizacion manual completada.", false);
            updateSyncMeta("manual");
        } catch (error) {
            console.error("Error al sincronizar manualmente:", error);
            setStatus("No se pudo sincronizar manualmente.", true);
        }
    }
    ...
    function bindEvents() {
        ...
        const forceSyncBtn = getEl("btn-force-sync");
        if (forceSyncBtn) {
    ...
    async function init() {
        ...
        try {
            ...
            if (!db
                ...
                || typeof db.getAllConfirmations !== "function"
