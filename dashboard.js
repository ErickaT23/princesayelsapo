...
function buildRows(confirmations, guestDirectory) {
    ...
    confirmationById.forEach((confirmation, id) => {
        ...
        rows.push({ ...confirmation, activo: true });
    });
...
function renderDesktopTable(rows, emptyMessage) {
    ...
    rows.forEach((row) => {
        ...
        nameTd.textContent = (row.nombre || "--") + (row.activo === false ? " (inactivo)" : "");
...
function renderMobileCards(rows, emptyMessage) {
    ...
    rows.forEach((row) => {
        ...
        nameMain.textContent = (row.nombre || "--") + (row.activo === false ? " (inactivo)" : "");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const syncButton = document.getElementById("dashboard-sync");
    ...
    async function forceSyncNow() {
        ...
        try {
            const [confirmations, invitados] = await Promise.all([
                window.RSVPDatabase.getAllConfirmations(activeEventId),
                window.RSVPDatabase.getInvitados(activeEventId)
            ]);
    ...
    if (syncButton) {
        syncButton.addEventListener("click", forceSyncNow);
    }
